...
//#define DEBUG_SENSOR_GC0328
static DEFINE_SPINLOCK(GC0328_drv_lock);
#define GC0328_TEST_PATTERN_CHECKSUM (0xb04ea30e)//(0xa2054b84)//0x12345678
...
#ifdef DEBUG_SENSOR_GC0328
#define gc0328_OP_CODE_INI		0x00		/* Initial value. */
#define gc0328_OP_CODE_REG		0x01		/* Register */
#define gc0328_OP_CODE_DLY		0x02		/* Delay */
#define gc0328_OP_CODE_END		0x03		/* End of initial setting. */
...
} gc0328_initial_set_struct;

gc0328_initial_set_struct gc0328_Init_Reg[5000];

static u32 strtol(const char *nptr, u8 base)
{

	printk("gc0328___%s____\n",__func__); 

	u8 ret;
	if(!nptr || (base!=16 && base!=10 && base!=8))
	{
		printk("gc0328 %s(): NULL pointer input\n", __FUNCTION__);
		return -1;
	}
	for(ret=0; *nptr; nptr++)
	{
...
static u8 GC0328_Initialize_from_T_Flash()
{
	...
	printk("gc0328___%s____11111111111111\n",__func__); 
	...
	fp = filp_open("/mnt/sdcard/gc0328_sd.txt", O_RDONLY , 0); 
	if (IS_ERR(fp)) 
	{ 
		printk("0328 create file error 1111111\n");  
		return -1; 
	} 
	else
	{
		printk("0328 create file error 2222222\n");  
	}
	fs = get_fs(); 
	set_fs(KERNEL_DS); 
	...
	printk("gc0328___%s____22222222222222222\n",__func__); 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			gc0328_Init_Reg[i].op_code = gc0328_OP_CODE_REG;

			gc0328_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			gc0328_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			curr_ptr += 4;	
			gc0328_Init_Reg[i].op_code = gc0328_OP_CODE_DLY;

			gc0328_Init_Reg[i].init_reg = 0xFF;
			gc0328_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	gc0328_Init_Reg[i].op_code = gc0328_OP_CODE_END;
	gc0328_Init_Reg[i].init_reg = 0xFF;
	gc0328_Init_Reg[i].init_val = 0xFF;
	i++;
	//for (j=0; j<i; j++)
	printk("gc0328 %x  ==  %x\n",gc0328_Init_Reg[j].init_reg, gc0328_Init_Reg[j].init_val);
	
	printk("gc0328___%s____3333333333333333\n",__func__); 

	/* Start apply the initial setting to sensor. */
#if 1
	for (j=0; j<i; j++)
	{
		if (gc0328_Init_Reg[j].op_code == gc0328_OP_CODE_END)	/* End of the setting. */
		{
			printk("gc0328 REG OK -----------------END!\n");
		
			break ;
		}
		else if (gc0328_Init_Reg[j].op_code == gc0328_OP_CODE_DLY)
		{
			msleep(gc0328_Init_Reg[j].init_val);		/* Delay */
			printk("gc0328 REG OK -----------------DLY!\n");			
		}
		else if (gc0328_Init_Reg[j].op_code == gc0328_OP_CODE_REG)
		{

			GC0328_write_cmos_sensor(gc0328_Init_Reg[j].init_reg, gc0328_Init_Reg[j].init_val);
			printk("gc0328 REG OK!-----------------REG(0x%x,0x%x)\n",gc0328_Init_Reg[j].init_reg, gc0328_Init_Reg[j].init_val);			
			printk("gc0328 REG OK!-----------------REG(0x%x,0x%x)\n",gc0328_Init_Reg[j].init_reg, gc0328_Init_Reg[j].init_val);			
			printk("gc0328 REG OK!-----------------REG(0x%x,0x%x)\n",gc0328_Init_Reg[j].init_reg, gc0328_Init_Reg[j].init_val);			
...
static kal_uint32 GC0328_g_fPV_PCLK = 26*1000000;
...
void GC0328_Sensor_Init(void)
{
	...
	GC0328_write_cmos_sensor(0x31, 0x06);   //exp level 3  7.14fps
	GC0328_write_cmos_sensor(0x32, 0x00); 
	GC0328_write_cmos_sensor(0xfe, 0x00);
	...
	GC0328_write_cmos_sensor(0x4d , 0x36);     ////////////40  
	GC0328_write_cmos_sensor(0x4e , 0x02);     //SPL D65 
	GC0328_write_cmos_sensor(0x4e , 0x02);     //SPL D65 
	GC0328_write_cmos_sensor(0x4d , 0x44);     ////////////40
	GC0328_write_cmos_sensor(0x4e , 0x02);
	GC0328_write_cmos_sensor(0x4e , 0x02);
	GC0328_write_cmos_sensor(0x4e , 0x02);     //SPL D65
	GC0328_write_cmos_sensor(0x4e , 0x02);      //SPL D65
	GC0328_write_cmos_sensor(0x4d , 0x53);     /////////////50
	GC0328_write_cmos_sensor(0x4e , 0x08);    //SPL CWF
	GC0328_write_cmos_sensor(0x4e , 0x08);      //DNP
	GC0328_write_cmos_sensor(0x4e , 0x02);   //04//pink      //DNP
	GC0328_write_cmos_sensor(0x4d , 0x63);     /////////////60
	GC0328_write_cmos_sensor(0x4e , 0x08);  	//TL84
	GC0328_write_cmos_sensor(0x4e , 0x08);  	//TL84
	GC0328_write_cmos_sensor(0x4d , 0x73);     //////////////80
	...
	GC0328_write_cmos_sensor(0x23 , 0x04); // 0x06 20140519 lanking GC0328C  
	...
	GC0328_write_cmos_sensor(0x41 , 0x26);   
	...
	GC0328_write_cmos_sensor(0x80 , 0x27); //  20140915
	GC0328_write_cmos_sensor(0x81 , 0x15); 
	GC0328_write_cmos_sensor(0x82 , 0x90);
	GC0328_write_cmos_sensor(0x83 , 0x02);
	GC0328_write_cmos_sensor(0x84 , 0x23); // 0x22 20140915
	GC0328_write_cmos_sensor(0x90 , 0x2c); 
	GC0328_write_cmos_sensor(0x92 , 0x02);
	GC0328_write_cmos_sensor(0x94 , 0x02);
	GC0328_write_cmos_sensor(0x95 , 0x35);  
	
	///////YCP
	GC0328_write_cmos_sensor(0xd1 , 0x32);
	GC0328_write_cmos_sensor(0xd2 , 0x32);
	GC0328_write_cmos_sensor(0xdd , 0x18);
	GC0328_write_cmos_sensor(0xde , 0x32);
	...
	GC0328_write_cmos_sensor(0xbf , 0x10);
	GC0328_write_cmos_sensor(0xc0 , 0x1c);
	GC0328_write_cmos_sensor(0xc1 , 0x33);
	GC0328_write_cmos_sensor(0xc2 , 0x48);
	GC0328_write_cmos_sensor(0xc3 , 0x5a);
	GC0328_write_cmos_sensor(0xc4 , 0x6b);
	GC0328_write_cmos_sensor(0xc5 , 0x7b);
	...
	GC0328_write_cmos_sensor(0xc7 , 0xab);
	GC0328_write_cmos_sensor(0xc8 , 0xbf);
	GC0328_write_cmos_sensor(0xc9 , 0xcd);
	GC0328_write_cmos_sensor(0xca , 0xd9);
	GC0328_write_cmos_sensor(0xcb , 0xe3);
	GC0328_write_cmos_sensor(0xcc , 0xeb);
	GC0328_write_cmos_sensor(0xcd , 0xf7);
	GC0328_write_cmos_sensor(0xce , 0xfd);
	GC0328_write_cmos_sensor(0xcf , 0xff);
	...
	GC0328_write_cmos_sensor(0x65 , 0x0c);    
	GC0328_write_cmos_sensor(0x66 , 0x1a);    
	GC0328_write_cmos_sensor(0x67 , 0x29);    
	GC0328_write_cmos_sensor(0x68 , 0x39);    
	GC0328_write_cmos_sensor(0x69 , 0x4b);    
	GC0328_write_cmos_sensor(0x6a , 0x5e);    
	GC0328_write_cmos_sensor(0x6b , 0x82);    
	GC0328_write_cmos_sensor(0x6c , 0xa4);    
	GC0328_write_cmos_sensor(0x6d , 0xc5);    
	GC0328_write_cmos_sensor(0x6e , 0xe5);    
	...
	GC0328_write_cmos_sensor(0x13 , 0x80);    ///  lanking +2
	GC0328_write_cmos_sensor(0x15 , 0xfc);   
	GC0328_write_cmos_sensor(0x18 , 0x03);
	GC0328_write_cmos_sensor(0x21 , 0xb0);   
	GC0328_write_cmos_sensor(0x22 , 0x40);   
	...
	GC0328_write_cmos_sensor(0xc1 , 0x0c);
	GC0328_write_cmos_sensor(0xc2 , 0x0a);
	GC0328_write_cmos_sensor(0xc6 , 0x0e);
	GC0328_write_cmos_sensor(0xc7 , 0x0b);
	GC0328_write_cmos_sensor(0xc8 , 0x0a);
	GC0328_write_cmos_sensor(0xba , 0x26);
	GC0328_write_cmos_sensor(0xbb , 0x1c);
	GC0328_write_cmos_sensor(0xbc , 0x1d);
	GC0328_write_cmos_sensor(0xb4 , 0x23);
	GC0328_write_cmos_sensor(0xb5 , 0x1c);
	GC0328_write_cmos_sensor(0xb6 , 0x1a);
	...
	GC0328_write_cmos_sensor(0xbd , 0x00);
	GC0328_write_cmos_sensor(0xbe , 0x00);
	GC0328_write_cmos_sensor(0xbf , 0x00);
	GC0328_write_cmos_sensor(0xb7 , 0x07);
	GC0328_write_cmos_sensor(0xb8 , 0x05);
	GC0328_write_cmos_sensor(0xb9 , 0x05);
	GC0328_write_cmos_sensor(0xa8 , 0x07);
	GC0328_write_cmos_sensor(0xa9 , 0x06);
	GC0328_write_cmos_sensor(0xaa , 0x00);
	GC0328_write_cmos_sensor(0xab , 0x04);
	GC0328_write_cmos_sensor(0xac , 0x00);
	GC0328_write_cmos_sensor(0xad , 0x02);
	GC0328_write_cmos_sensor(0xae , 0x0d);
	GC0328_write_cmos_sensor(0xaf , 0x05);
	GC0328_write_cmos_sensor(0xb0 , 0x00);
	GC0328_write_cmos_sensor(0xb1 , 0x07);
	GC0328_write_cmos_sensor(0xb2 , 0x03);
	GC0328_write_cmos_sensor(0xb3 , 0x00);
	...
	GC0328_write_cmos_sensor(0xB2 , 0xfd);   
	GC0328_write_cmos_sensor(0xB3 , 0xfc);   
	GC0328_write_cmos_sensor(0xB4 , 0xf0);   
	GC0328_write_cmos_sensor(0xB5 , 0x05);   
	GC0328_write_cmos_sensor(0xB6 , 0xf0);   
	/*   /////skin CC for front //////
	GC0328_write_cmos_sensor(0xb1 , 0x00);
	GC0328_write_cmos_sensor(0xb2 , 0x00);
	GC0328_write_cmos_sensor(0xb3 , 0x00);
	GC0328_write_cmos_sensor(0xb4 , 0xf0);
	GC0328_write_cmos_sensor(0xb5 , 0x00);
	GC0328_write_cmos_sensor(0xb6 , 0x00);
	*/
	Sleep(200);
	...
	GC0328_write_cmos_sensor(0x7d , 0x00);   
	GC0328_write_cmos_sensor(0x50 , 0x88);   
	GC0328_write_cmos_sensor(0x5b , 0x0c); 
	...
	GC0328_write_cmos_sensor(0x70 , 0xff); 
	GC0328_write_cmos_sensor(0x71 , 0x00); 
	GC0328_write_cmos_sensor(0x72 , 0x28); 
	GC0328_write_cmos_sensor(0x73 , 0x0b); 
	GC0328_write_cmos_sensor(0x74 , 0x0b); 
...
UINT32 GC0328Open(void)
{
    ...
    SENSORDB("Sensor Read ID OK,ID=0x%x \r\n",sensor_id);
    ...
    GC0328_Write_More_Registers();
		...
		{
			fromsd = 1;
			printk("open 0328 file ok\n");
			printk("open 0328 file ok\n");
			printk("open 0328 file ok\n");

			//gc0328_Initialize_from_T_Flash();
		...
		if(fromsd == 1)
		{
			printk("________________0328 from t!\n");
			printk("________________0328 from t!\n");
			printk("________________0328 from t!\n");		
			GC0328_Initialize_from_T_Flash();
			printk("______after_____0328 from t!\n");	
			//GC0328_Sensor_Init(); 	
		}
		else
		{
			//GC0328_MPEG4_encode_mode = KAL_FALSE;
			printk("________________0328 not from t!\n");	
			printk("________________0328 not from t!\n");
			printk("________________0328 not from t!\n");		
			SENSORDB"Sensor Read ID OK \r\n"); 
		}
...
UINT32 GC0328MaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
	{
		kal_uint32 pclk;
		kal_int16 dummyLine;
		kal_uint16 lineLength,frameHeight;
		SENSORDB("GC0328MaxFramerateByScenario: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
		SENSORDB("[GC0328]enter GC0328MaxFramerateByScenario function:\n ");
		switch (scenarioId) {
			case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
				...
				spin_lock(&GC0328_drv_lock);
				//GC0328_SensorDriver.SensorMode= SENSOR_MODE_PREVIEW;
				GC0328_dummy_lines = dummyLine;
				spin_unlock(&GC0328_drv_lock);
				//GC0328SetDummy(GC0328_dummy_lines, GC0328_dummy_pixels);			
				break;			
		...
		SENSORDB("[GC0328]exit GC0328MaxFramerateByScenario function:\n ");
		return ERROR_NONE;
	}
UINT32 GC0328GetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
	SENSORDB("[GC0328]enter GC0328GetDefaultFramerateByScenario function:\n ");
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			 *pframeRate = 300;
	...
	SENSORDB("[GC0328]exit GC0328GetDefaultFramerateByScenario function:\n ");
	return ERROR_NONE;
}
...
BOOL GC0328_set_param_exposure(UINT16 para)
{
	switch (para)
	{
		...
		case AE_EV_COMP_00:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0x80);  //////// lanking
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		...
		case AE_EV_COMP_13:
			...
			GC0328_write_cmos_sensor(0xfe, 0x01);
			GC0328_write_cmos_sensor(0x13, 0xa0);  //  lanking
			GC0328_write_cmos_sensor(0xfe, 0x00);
		break;
		default:
		return ERROR_INVALID_CTRL_CODE;
...
#ifdef DEBUG_SENSOR_GC0328
		printk("______%s______GC0328 YUV setting\n",__func__);
		return TRUE;
#endif
...
void GC0328YUVGetDelayInfo(uintptr_t delayAddr)
{
...
GC0328YUV_AF_Get_Max_Num_Focus_Areas(UINT32 *feature_data)
{
    *feature_data = 0;
...
static void
GC0328YUV_AE_Get_Max_Num_Metering_Areas(UINT32 *feature_data)
{
    *feature_data = 0;
}

UINT32 GC0328SetTestPatternMode(kal_bool bEnable)
{
	GC0328_write_cmos_sensor(0xfe,0x00);
                                  
GC0328_write_cmos_sensor(0xfa,0x32);//24MCLK  ///0x11 for 12MCLK
	                               
GC0328_write_cmos_sensor(0x40,0x08);//only intpenable
GC0328_write_cmos_sensor(0x41,0x00);//close all
GC0328_write_cmos_sensor(0x42,0x00);//close all
GC0328_write_cmos_sensor(0x4f,0x00);//AEC un
	                               
GC0328_write_cmos_sensor(0x18,0x06);//sdark
                                 
GC0328_write_cmos_sensor(0x77,0x40);//AWB gain
GC0328_write_cmos_sensor(0x78,0x40);//AWB gain
GC0328_write_cmos_sensor(0x79,0x40);//AWB gain
GC0328_write_cmos_sensor(0xd0,0x40);//asdesaturation
GC0328_write_cmos_sensor(0xdd,0x00);//edge_sa
GC0328_write_cmos_sensor(0x71,0x40);//Auto_pregain
GC0328_write_cmos_sensor(0x72,0x40);//Auto_pregain
GC0328_write_cmos_sensor(0x03, 0x00);//Exposure 
GC0328_write_cmos_sensor(0x04,0x00);          
                                 
GC0328_write_cmos_sensor(0x4c,0x01);//test pattern
        ...
        UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    SENSORDB("gaiyang GC0328FeatureControl FeatureId=%d\r\n", FeatureId);
    ...
    case SENSOR_FEATURE_SET_ISP_MASTER_CLOCK_FREQ:
        GC0328_isp_master_clock=*feature_data;
        break;
    ...
    case SENSOR_FEATURE_GET_LENS_DRIVER_ID:
        ...
        *pFeatureParaLen=4;
		...
		case SENSOR_FEATURE_SET_TEST_PATTERN:			 
			GC0328SetTestPatternMode((BOOL)*feature_data);			
			break;
		case SENSOR_FEATURE_GET_TEST_PATTERN_CHECKSUM_VALUE:
			*pFeatureReturnPara32=GC0328_TEST_PATTERN_CHECKSUM;
			*pFeatureParaLen=4;
			break;
    case SENSOR_FEATURE_SET_YUV_CMD:
        GC0328YUVSensorSetting((FEATURE_ID)*feature_data, *(feature_data+1));
        break;
    case SENSOR_FEATURE_SET_VIDEO_MODE:    //  lanking
				 GC0328YUVSetVideoMode(*feature_data);
	 			break;
    case SENSOR_FEATURE_CHECK_SENSOR_ID:
					GC0328GetSensorID(feature_data);
					break;
		case SENSOR_FEATURE_SET_MAX_FRAME_RATE_BY_SCENARIO:
			GC0328MaxFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*feature_data, *(feature_data+1));//((MSDK_SCENARIO_ID_ENUM)*pFeatureData32,*(pFeatureData32+1));
			break;
		case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
			GC0328GetDefaultFramerateByScenario((MSDK_SCENARIO_ID_ENUM)*feature_data, (MUINT32 *)(uintptr_t)(*(feature_data+1)));//((MSDK_SCENARIO_ID_ENUM)*pFeatureData32,(MUINT32 *)*(pFeatureData32+1));
			break;
    case SENSOR_FEATURE_GET_DELAY_INFO:
        SENSORDB("[GC0328] F_GET_DELAY_INFO\n");
        GC0328YUVGetDelayInfo((uintptr_t)*feature_data);         
    		break;	
  	case SENSOR_FEATURE_GET_AF_MAX_NUM_FOCUS_AREAS:
        GC0328YUV_AF_Get_Max_Num_Focus_Areas(feature_data);           
        *pFeatureParaLen=4;
        break;    
    case SENSOR_FEATURE_GET_AE_MAX_NUM_METERING_AREAS:
         GC0328YUV_AE_Get_Max_Num_Metering_Areas(feature_data);
       SENSORDB("[GC0328] GC0328YUV_AE_Get_Max_Num_Metering_Areas=%d\n", *pFeatureReturnPara32);
        *pFeatureParaLen=4;    				
    default:
        break;
	}
