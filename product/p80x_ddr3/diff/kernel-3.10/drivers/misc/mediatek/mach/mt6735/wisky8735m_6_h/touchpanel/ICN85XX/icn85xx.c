...
dma_addr_t *icn85xx_i2c_dma_pa = NULL; // dma physical address
...
static ssize_t icn85xx_tool_read(struct file *file, char __user * buffer, size_t count, loff_t * ppos)
{
	...
	proc_info("%s: count:%zu, off:%d, cmd_head.data_len: %d\n",__func__, count,(int)(* ppos),(int)cmd_head.data_len); 
	if (cmd_head.wr % 2)
	{
		ret = 0;
...
read_out:
	up(&icn85xx_ts->sem);   
	proc_info("%s out: %zu, cmd_head.data_len: %d\n\n",__func__, count, cmd_head.data_len); 
...
int icn85xx_prog_i2c_txdata(unsigned int addr, char *txdata, int length)
{
	...
	{
		icn85xx_error("i2c_client or icn85xx_i2c_dma_va is null pointer\n");
		return -1;
	}
...
static int icn85xx_i2c_test(void)
{
	...
	while(retry++ < 3)
	{
		...
		icn85xx_trace("icn85xx_check_progmod: 0x%x,0x%x,0x%x\n", buf[0],buf[1],buf[2]);
		if(ret > 0)
		{
			//if(value == 0x85)
...
static int  tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int keyarr[]={KEY_O,KEY_W,KEY_M,KEY_E,KEY_C,KEY_UP,KEY_DOWN,KEY_LEFT,KEY_RIGHT,KEY_U};
	int retval = TPD_OK;
	int err = 0;
	short fwVersion = 0;
	short curVersion = 0;
